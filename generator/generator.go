...
func init() {
	android.RegisterModuleType("benzo_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = benzoExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = benzoExpandVariables(ctx, dep_file)
	...
	cmd := benzoExpandVariables(ctx, String(g.properties.Cmd))
